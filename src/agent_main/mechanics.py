...
def predict_orbit_position(pos, t, omega):
    cx, cy = SUN_POS
    x, y = pos
...
def predict_target(pos, t, omega):
...
def is_inner_planet(pos, radius):
    ...
    cx, cy = SUN_POS
    x, y = pos
    orbital_radius = math.hypot(x - cx, y - cy)
    return orbital_radius + radius < 50.0

def intercept_time(source, target_pos, omega, speed, max_t=500, iters=250):
    # binary search for consistent travel time
    lo, hi = 0.0, max_t

    for _ in range(iters):
        mid = (lo + hi) / 2

        tx, ty = predict_target(target_pos, mid, omega)

        dist = math.hypot(tx - source[0], ty - source[1])
        travel = dist / speed

        # if we arrive too late, need more time
        if travel > mid:
            lo = mid
        else:
            hi = mid
